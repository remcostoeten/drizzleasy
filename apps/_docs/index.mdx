...
title: Drizzleasy
description:
    Ultra-simple, type-safe CRUD operations for Next.js with Drizzle ORM. Build database operations
...
Ultra-simple, type-safe CRUD operations for Next.js with Drizzle ORM. Build database operations with
full TypeScript support, IntelliSense, and error handling.
...
- 📝 **IntelliSense support** - Complete autocomplete and hover documentation
- ⚡ **Multiple databases** - PostgreSQL, MySQL, SQLite, Turso support
- 🎯 **Optimistic updates** - Client-side React hooks included
...
const { data: activeUsers } = await read('users').where({ status: 'active' })()
...
const { data: categoryProducts } = await read('products').where({
    category: ['books', 'electronics', 'clothing']
})()

// Not equal
const { data: availableProducts } = await read('products').where({ category: '!discontinued' })() // Not discontinued
```
