...
title: API Reference
description:
    Complete API documentation for CRUD Builder. Learn about all available methods, types, and
...
Reads records from a table with chainable WHERE conditions.

```typescript
crud.read<T>(tableName: string): {
  (): Promise<TResult<T[]>>  // Callable for all records
  where(condition: TWhereClause<T>): QueryBuilder<T>
...
- `()` - Get all records (callable) and executes accumulated WHERE conditions
...
```typescript
// Get all users
const { data: users } = await crud.read('users')()

// Get specific user
const { data: user } = await crud.read('users').byId('123')

// Filter with WHERE
const { data: filtered } = await crud
    .read('users')
    .where({ status: 'active' })
    .where({ age: '>18' })()
```
...
Creates a typed create function for reusable CRUD operations.
...
const { data, error } = await create('users')({ name: 'John', email: 'john@example.com' })
```

### readFn()

Creates a typed read function with WHERE filtering capabilities.
...
// Filter with WHERE
const { data: filtered } = await read('users').where({ status: 'active' })()
```

### updateFn()

Creates a typed update function for modifying existing records.
...
const { data, error } = await update('users')('123', { name: 'Jane' })
```

### destroyFn()

Creates a typed destroy function for deleting records.
...
const { data, error } = await destroy('users')('123')
```
...
```typescript
function useOptimisticCrud<T extends TEntity>(
    initialData: T[]
): {
    data: T[]
    isPending: boolean
