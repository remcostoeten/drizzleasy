...
title: WHERE Clauses
description:
    Simple and intuitive WHERE clauses with natural syntax. No complex operators or MongoDB-style
    queries.
...
// Simple equality
const activeUsers = await read('users').where({ status: 'active' })()

// Multiple conditions (AND)
const adminUsers = await read('users').where({ status: 'active' }).where({ role: 'admin' })()
```
...
const admins = await read('users')
    .where({ role: 'admin' })
    .where({ status: 'active' })
    .where({ createdAt: '>=2024-01-01' })()

// Gmail users
const gmailUsers = await read('users').where({ email: '*@gmail.com' })()
```
...
const read = readFn<Post>()

// Published React posts
const reactPosts = await read('posts').where({ status: 'published' }).where({ title: '*React*' })()
```
